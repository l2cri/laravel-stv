...
<div class="simple-drop-down">
    <select onchange="setLocation(this.value)">
        ...
        value="{{ url_add_params(array('dir='.$dir, $prefix.'order=name'), url($currentUrl)) }}">
            Названию                </option>
        <option @if($order == 'rating')selected="selected"@endif
        value="{{ url_add_params(array('dir='.$dir, $prefix.'order=rating'), url($currentUrl)) }}">
            Рейтингу                </option>
    </select>
</div>
@if($dir == 'asc')
    <a title="По убыванию" href="{{ url_add_params(array('dir=desc', $prefix.'order='.$order), url($currentUrl)) }}" class="sort-button">
        &nbsp;
    </a>
@else
    <a title="По возрастанию" href="{{ url_add_params(array('dir=asc', $prefix.'order='.$order), url($currentUrl)) }}" class="sort-button active">
        &nbsp;
    </a>
@endif